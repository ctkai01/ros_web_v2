...
class MissionButtonGroupWidget {
  ...
  startMission(missionId) {
    const mission = this.settings.missions?.find(
  ...
  updateMissionExecutionStatus(missionId, status) {
    const mission = this.settings.groupId?.find(
      (m) => m.missionId === missionId
    );
    if (mission) {
  ...
  render(onEdit) {
    return React.createElement(MissionButtonGroupWidgetComponent, {
      widget: this,
      onEdit,
    });
  }
