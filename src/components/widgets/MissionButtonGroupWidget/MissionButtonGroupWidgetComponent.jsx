...
const MissionButtonGroupWidgetComponent = ({ widget, onEdit }) => {
  ...
  const loadGroupMissions = async (groupId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${serverConfig.SERVER_URL}/api/missions/group/${groupId}`,
        {
          method: "GET",
          headers: {
  ...
  const handleMissionClick = async (mission) => {
    ...
    if (executingMissions.has(missionId) || pendingRequests.has(missionId)) {
      console.log(
        "Mission đang thực thi hoặc request đang pending, bỏ qua click"
      );
      return;
    }
        ...
        {(() => {
          ...
          if (widget.settings.groupId) {
            ...
            if (groupMissions.length === 0) {
              return (
                <Card className="col-span-3 lg:col-span-1">
                  <p className="text-center">No missions found in group</p>
                </Card>
              );
            }
                  ...
                  {groupMissions.map((mission, index) => {
                    ...
                    return (
                      <MissionButton
                        ...
                        icon={<TbTargetArrow />}
                        label={mission.name || `Mission ${index + 1}`}
          ...
          if (
            !widget.settings.missions ||
            widget.settings.missions.length === 0
          ) {
            return (
          ...
          const validMissions = widget.settings.missions.filter(
              ...
              {validMissions.map((mission, index) => {
                ...
                return (
                  <div
                    ...
                    className="mission-button-item"
                  >
                    <button
                      className={`mission-btn ${getButtonSizeClass()} ${
                      ...
                      <span className="button-text">
                        {(() => {
                          ...
                          if (buttonText && typeof buttonText === "string") {
                            ...
                            return (
                              buttonText.name || buttonText.title || "Mission"
                            );
                          } else if (
                            missionName &&
                            typeof missionName === "object"
                          ) {
                            return (
                              missionName.name || missionName.title || "Mission"
                            );
                          } else {
