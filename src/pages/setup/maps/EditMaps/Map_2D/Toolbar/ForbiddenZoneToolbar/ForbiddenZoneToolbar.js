...
import { DrawForbiddenZone } from './drawForbiddenZone.js';
import './ForbiddenZoneToolbar.css';

class ForbiddenZoneToolbar {
    constructor(map2D) {
        this.map2D = map2D;
        this.container = null;
    ...
    createToolbar() {
        ...
        this.container.className = 'forbidden-zone-tools';
        ...
        if (toolbarLeftGroup) {
            ...
            if (mapToolbar) {
                ...
                if (this.map2D.container) {
                    ...
                    console.error('No suitable container found for ForbiddenZoneToolbar');
                }
            }
        }
    }
    ...
    createDrawZoneSection() {
        // Draw zone button
        this.tools.drawZone.button = this.createButton('Draw Forbidden Zone', 'draw-forbidenzones-icon', 'forbidden-zone-draw-button');
        this.container.appendChild(this.tools.drawZone.button);
    }

    createCursorSection() {
        // Cursor button
        this.tools.cursor.button = this.createButton('Cursor', 'cursor-icon', 'forbidden-zone-cursor-button');
    ...
    createActionButtons() {
        ...
        this.actionButtons.cancel = this.createButton('Cancel changes', 'cancel-changes-icon', 'forbidden-zone-cancel-changes-button');
        this.actionButtons.apply = this.createButton('Apply changes', 'apply-changes-icon', 'forbidden-zone-apply-changes-button');
        this.actionButtons.edit = this.createButton('Edit zone', 'forbidden-zone-edit-icon', 'forbidden-zone-edit-button');
        this.actionButtons.delete = this.createButton('Delete zone', 'delete-icon', 'forbidden-zone-delete-button');
    ...
    createButton(title, iconClass, id) {
        ...
        if (id === 'forbidden-zone-cancel-changes-button') {
        ...
        } else if (id === 'forbidden-zone-apply-changes-button') {
    ...
    bindEvents() {
        ...
        // Action buttons
        this.actionButtons.cancel.addEventListener('click', () => this.cancelChangesForbiddenZone());
    ...
    handleDrawZone() {
        ...
        this.map2D.setToolMapMode(true, this.map2D.MouseMode.DRAW_FORBIDDEN_ZONE);
        
        // Enable drawing in the DrawForbiddenZone tool
        if (this.map2D.toolDrawForbiddenZone) {
            this.map2D.toolDrawForbiddenZone.setMode('create');
            this.map2D.toolDrawForbiddenZone.enable();
        }
    ...
    handleCursor() {
        ...
        this.map2D.setToolMapMode(true, this.map2D.MouseMode.SELECT_FORBIDDEN_ZONE);
        
        // Enable select mode in the DrawForbiddenZone tool
        if (this.map2D.toolDrawForbiddenZone) {
            this.map2D.toolDrawForbiddenZone.setMode('select');
            this.map2D.toolDrawForbiddenZone.enable();
        }
    ...
    cancelChangesForbiddenZone() {
        this.map2D.cancelMapTool();
        this.hideActionButtons();
        this.deactivateAllTools();
        
        if (this.map2D.toolDrawForbiddenZone) {
            this.map2D.toolDrawForbiddenZone.clearCurrentState();
            this.map2D.toolDrawForbiddenZone.disable();
        }
    }
    ...
    handleEditZone() {
        if (this.map2D && this.map2D.toolDrawForbiddenZone) {
            this.map2D.toolDrawForbiddenZone.editSelectedPolygon();
    ...
    handleDeleteZone() {
        if (this.map2D && this.map2D.toolDrawForbiddenZone) {
            this.map2D.toolDrawForbiddenZone.deleteSelectedPolygon();
            this.hideEditActions();
        }
    }
    ...
    deactivateAllTools() {
        ...
        if (this.map2D && this.map2D.toolDrawForbiddenZone) {
            this.map2D.toolDrawForbiddenZone.disableSelectMode();
        }
    }
    ...
    show() {
        console.log('ForbiddenZoneToolbar: show() called');
        this.isVisible = true;
    ...
    updateFromMap2D() {
        if (this.map2D.currentMouseMode === this.map2D.MouseMode.DRAW_FORBIDDEN_ZONE) {
            this.tools.drawZone.isActive = true;
            this.tools.drawZone.button.classList.add('active');
        } else if (this.map2D.currentMouseMode === this.map2D.MouseMode.SELECT_FORBIDDEN_ZONE) {
            this.tools.cursor.isActive = true;
            this.tools.cursor.button.classList.add('active');
        }
    }
