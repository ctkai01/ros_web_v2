import { DrawCriticalZone } from "./drawCriticalZone.js";
import "./CriticalZoneToolbar.css";

class CriticalZoneToolbar {
  constructor(map2D) {
    this.map2D = map2D;
    this.container = null;
  ...
  createToolbar() {
    ...
    this.container.className = "critical-zone-tools";
    ...
    if (toolbarLeftGroup) {
      ...
      if (mapToolbar) {
        ...
        if (this.map2D.container) {
          ...
          console.error("No suitable container found for CriticalZoneToolbar");
        }
      }
    }
  }
  ...
  createDrawZoneSection() {
    ...
    this.tools.drawZone.button = this.createButton(
      "Draw Critical Zone",
      "draw-criticalzones-icon",
      "critical-zone-draw-button"
    );
  ...
  createLineSettingsSection() {
    ...
    this.tools.lineSettings.button = this.createButton(
      "Lines Settings",
      "line-settings-critical-zones-icon",
      "critical-zone-line-settings-button"
    );
    ...
    options.forEach((option) => {
      ...
      optionButton.addEventListener("click", () => {
        ...
        this.map2D.toolDrawCriticalZone.setCurrentStyle(option.style);
        // }
  ...
  createActionButtons() {
    ...
    this.actionButtons.cancel = this.createButton(
      "Cancel changes",
      "cancel-changes-icon",
      "critical-zone-cancel-changes-button"
    );
    this.actionButtons.apply = this.createButton(
      "Apply changes",
      "apply-changes-icon",
      "critical-zone-apply-changes-button"
    );
    this.actionButtons.edit = this.createButton(
      "Edit zone",
      "critical-zone-edit-icon",
      "critical-zone-edit-button"
    );
    this.actionButtons.delete = this.createButton(
      "Delete zone",
      "delete-icon",
      "critical-zone-delete-button"
    );
  ...
  createButton(title, iconClass, id) {
    ...
    if (id === "critical-zone-cancel-changes-button") {
    ...
    } else if (id === "critical-zone-apply-changes-button") {
  ...
  bindEvents() {
    ...
    this.actionButtons.cancel.addEventListener("click", () =>
      this.cancelChangesCriticalZone()
    );
  ...
  handleDrawZone() {
    ...
    this.map2D.setToolMapMode(true, this.map2D.MouseMode.DRAW_CRITICAL_ZONE);

    // Enable drawing in the DrawCriticalZone tool
    if (this.map2D.toolDrawCriticalZone) {
      this.map2D.toolDrawCriticalZone.setMode("create");
      this.map2D.toolDrawCriticalZone.enable();
    }
  ...
  handleLineSettings() {
    ...
    if (this.lineSettingsPopup.style.display === "none") {
      ...
      const currentStyle = this.map2D.toolDrawCriticalZone.getCurrentStyle();
  ...
  handleCursor() {
    ...
    this.map2D.setToolMapMode(true, this.map2D.MouseMode.SELECT_CRITICAL_ZONE);

    // Enable select mode in the DrawCriticalZone tool
    if (this.map2D.toolDrawCriticalZone) {
      this.map2D.toolDrawCriticalZone.setMode("select");
      this.map2D.toolDrawCriticalZone.enable();
    }
  ...
  cancelChangesCriticalZone() {
    this.map2D.cancelMapTool();
    this.hideActionButtons();
    this.deactivateAllTools();

    if (this.map2D.toolDrawCriticalZone) {
      this.map2D.toolDrawCriticalZone.clearCurrentState();
      this.map2D.toolDrawCriticalZone.disable();
    }
  }
  ...
  handleEditZone() {
    if (this.map2D && this.map2D.toolDrawCriticalZone) {
      this.map2D.toolDrawCriticalZone.editSelectedLine();
      //   this.map2D.toolDrawCriticalZone.editSelectedPolygon();
  ...
  handleDeleteZone() {
    if (this.map2D && this.map2D.toolDrawCriticalZone) {
      this.map2D.toolDrawCriticalZone.deleteSelectedLine();
      //   this.map2D.toolDrawCriticalZone.deleteSelectedPolygon();
      this.hideEditActions();
    }
  }
  ...
  deactivateAllTools() {
    ...
    if (this.map2D && this.map2D.toolDrawCriticalZone) {
      this.map2D.toolDrawCriticalZone.disableSelectMode();
    }
  }
  ...
  show() {
    console.log("CriticalZoneToolbar: show() called");
    this.isVisible = true;
  ...
  updateFromMap2D() {
    if (
      this.map2D.currentMouseMode === this.map2D.MouseMode.DRAW_CRITICAL_ZONE
    ) {
      this.tools.drawZone.isActive = true;
      this.tools.drawZone.button.classList.add("active");
    } else if (
      this.map2D.currentMouseMode === this.map2D.MouseMode.SELECT_CRITICAL_ZONE
    ) {
      this.tools.cursor.isActive = true;
      this.tools.cursor.button.classList.add("active");
    }
