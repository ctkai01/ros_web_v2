...
import { Map2D } from "../../../setup/maps/EditMaps/Map_2D/Map_2D";
...
const MapWidgetComponent = ({ widget, onEdit }) => {
  ...
  const reloadMapData = async (newMapId, newSiteId) => {
    try {
      console.log('🔄 MapWidget: Reloading map data due to map/site change:', { newMapId, newSiteId });
      ...
      if (mapData) {
        ...
        if (mapRef.current && mapData) {
          ...
          if (mapRef.current) {
            console.log('🔄 MapWidget: Reloading all map elements in parallel...');
            ...
            console.log(`✅ MapWidget: All map elements reloaded in ${(endTime - startTime).toFixed(2)}ms`);
          }
  ...
  const getMapDataFromServer = async (mapId, siteId) => {
    try {
      ...
      console.log('🔄 MapWidget: Loading map data for mapId:', mapId, 'siteId:', siteId);

      // First try to load map data
      const response = await fetch(`${serverConfig.SERVER_URL}/api/maps/load/${mapId}`, {
        method: 'POST',
        headers: {
      ...
      if (data.success && data.data) {
        console.log('✅ MapWidget: API response is successful, validating data structure...');
        ...
        if (!data.data.info || !data.data.mapData) {
          console.error('❌ MapWidget: Invalid map data structure - missing info or mapData');
          return null;
        }
        ...
        try {
          ...
          try {
            ...
            const startNavigationResponse = await fetch(`${serverConfig.SERVER_URL}/api/robot/start-navigation`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            ...
            console.log('📡 MapWidget: start-navigation response received:', startNavigationResponse.status);
            if (startNavigationResponse.ok) {
              console.log('✅ MapWidget: Navigation started successfully');
            } else {
              console.warn('⚠️ MapWidget: Failed to start navigation, status:', startNavigationResponse.status);
            }
          } catch (navError) {
          ...
          console.error('❌ MapWidget: Error fetching additional map data:', error);
  ...
  const getMapId = async () => {
    try {
      ...
      const response = await fetch(`${serverConfig.SERVER_URL}/api/maps/getCurrentMapId/${siteId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      if (widget.displayMode === 'display' && mapRef.current) {
        ...
        if (mapId) {
          console.log('🔄 MapWidget: About to call getMapDataFromServer with:', { mapId, siteId });
          const mapData = await getMapDataFromServer(mapId, siteId);
          console.log('📊 MapWidget: getMapDataFromServer returned:', mapData);
          if (mapData) {
            console.log('✅ MapWidget: mapData is valid, processing...', mapData);
            setGlobalCurrentMapData(mapData);
            ...
            if (mapRef.current && mapData) {
              ...
              if (mapRef.current) {
                console.log('🔄 MapWidget: Loading all map elements in parallel...');
                const startTime = performance.now();
                ...
                const endTime = performance.now();
                console.log(`✅ MapWidget: All map elements loaded in ${(endTime - startTime).toFixed(2)}ms`);
              }
  ...
  const handleInitPoseEstimate = async (poseEstimateData) => {
    ...
    try {
      ...
      const response = await fetch(`${serverConfig.SERVER_URL}/api/robot/init-pose-estimate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const startDisplayMode = () => {
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (message.type === 'map_change') {
            console.log('📋 MapWidget: Received map change notification:', message.data);

            const { id_map, id_site } = message.data;
            if (id_map == null || id_site == null) {
              console.log('📋 MapWidget: Map change notification received but no change detected');
              return;
            }
            ...
            if (id_map !== currentMapId || id_site !== currentSiteId) {
              console.log('🔄 MapWidget: Map or site changed, reloading data:', {
              ...
              console.log('ℹ️ MapWidget: Map change notification received but no change detected');
            }
          }
          ...
          // Handle laser scan updates
            ...
            if (mapRef.current.isInitialized &&
              ...
              console.warn('❌ Cannot process scan - conditions not met:', {
                ...
                robotHasScene: mapRef.current.robot ? !!mapRef.current.robot.scene : false,
                scenesMatch: mapRef.current.robot ? mapRef.current.robot.scene === mapRef.current.scene : false
              });
            }
          }
      ...
      ws.onclose = () => {
        ...
        if (mapRef.current && mapRef.current.displayRobot && wsRef.current === null) {
